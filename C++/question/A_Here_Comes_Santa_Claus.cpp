...
int main()
{
    ...
    //             p3 = ((arr[0] + arr[1]) / 2.0);
    //             p2 = ((arr[n - 1] + arr[n - 2]) / 2.0);
    //             p4 = ((arr[n - 1] + arr[n - 3]) / 2.0);
    //             solution1 = p2 - p1;
    //             solution2 = p4 - p3;
    //             solution1 = max(solution1, solution2);
    //         }
    //         else
    //         {
    //             p1 = ((arr[0] + arr[1]) / 2.0);
    ...
    while (n--)
    {
        ...
        cin >> cost >> a >> b >> c >> d >> e;
        int mn = INT_MAX, mx = 0;
        mn = min(a, min(b, min(c, min(d, e))));
        mx = max(a, max(b, max(c, max(d, e))));
        if (mp.find(mx - mn) != mp.end())
        {
            mp[mx - mn] = min(mp[mx - mn], cost);
        }
        else
        {
            mp[mx - mn] = cost;
    ...
    while (q--)
    {
        ...
        mn = min(a, min(b, min(c, min(d, e))));
        mx = max(a, max(b, max(c, max(d, e))));
        // cout << "query #" << cnt++ << ": " << mp[mx - mn] << endl;
        cout << "query #" << cnt++<< " "<<mx<< ": " << mn << endl;
    }
}
